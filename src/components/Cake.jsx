...
import { OrbitControls, Stars, Float, ContactShadows, Torus, Cylinder } from '@react-three/drei';
  ...
  return (
    <group position={position}>
      ...
      <mesh ref={flameRef} position={[0, 0.9, 0]}>
        ...
        <meshStandardMaterial color="#ffaa00" emissive="#ff5500" emissiveIntensity={3} />
      </mesh>
    </group>
  );
};
...
const CakeModel = ({ themeColor, onCut, isCut }) => {
  ...
  useFrame((state) => {
    ...
    if (isCut && sliceRef.current) {
        ...
        sliceRef.current.position.x = lerp(sliceRef.current.position.x, 1.5, 0.05);
        sliceRef.current.position.y = lerp(sliceRef.current.position.y, 0.5, 0.05);
        sliceRef.current.rotation.z = lerp(sliceRef.current.rotation.z, -0.2, 0.05);
    }
  });

  // Helper for Icing
  const IcingTorus = ({ y, scale }) => (
      <Torus args={[1.55, 0.1, 16, 32]} position={[0, y, 0]} rotation={[Math.PI/2, 0, 0]} scale={[scale, scale, 1]}>
    ...
    <group ref={cakeGroup} onClick={onCut}>
      ...
      <group>
          ...
          {[0, 60, 120, 180, 240, 300].map((angle, i) => (
             <mesh key={i} position={[Math.cos(angle * Math.PI/180)*1.3, 1.35, Math.sin(angle * Math.PI/180)*1.3]}>
                 <sphereGeometry args={[0.15, 16, 16]} />
                 <meshStandardMaterial color="#d00000" roughness={0.1} metalness={0.3} />
             </mesh>
          ))}
      </group>
  ...
  return (
    <div style={{ height: '100vh', width: '100vw', background: theme.colors.background }}>
      <div style={{
          ...
          <h1 style={{ fontFamily: 'Dancing Script', fontSize: '3.5rem', margin: 0 }}>
             Make a Wish...
          </h1>
      ...
      <Canvas shadows camera={{ position: [0, 2, 7], fov: 40 }} dpr={[1, 2]}>
        ...
        <spotLight position={[10, 10, 5]} angle={0.2} penumbra={1} intensity={2} castShadow color="#fff" />
        <pointLight position={[-10, 5, -5]} intensity={1} color={theme.colors.secondary} />
        ...
        <Float speed={2} rotationIntensity={0.2} floatIntensity={0.5}>
           <Suspense fallback={null}>
             <CakeModel themeColor={theme.modelColor} onCut={handleCut} isCut={isCut} />
           </Suspense>
        </Float>
        ...
        <div style={{
            ...
            <motion.h2
              ...
              style={{
                ...
                fontSize: '4rem',
                color: 'white',
                textShadow: '0 0 10px rgba(0,0,0,0.5), 0 0 20px ' + theme.colors.primary,
                marginBottom: '20px'
              }}
            >
